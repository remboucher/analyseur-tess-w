...
const DataAnalyzer = () => {
  ...
  const [selectedCondition, setSelectedCondition] = useState('avec_neige'); // v7.1: condition affichée
  const [p50YLimits, setP50YLimits] = useState({ min: 16, max: 23 }); // v7.1: limites axe Y
  ...
  const calculatePSIE = () => {
    ...
    // v7.1: Calculer les données pour le graphique "Effet des nuages"
    const cloudEffectData = years.map(year => {
      const yearResults = allResults.filter(r => r.annee === year);
      const degage = yearResults.find(r => r.type === 'Dégagé');
      const nuageux = yearResults.find(r => r.type === 'Nuageux');
      
      if (degage && nuageux && degage.p50 != null && nuageux.p50 != null) {
        return {
          annee: year,
          effet: nuageux.p50 - degage.p50,
          p50_degage: degage.p50,
          p50_nuageux: nuageux.p50
        };
      }
      return null;
    }).filter(d => d !== null);
    ...
    const p50EvolutionData = years.map(year => {
      const yearResults = allResults.filter(r => r.annee === year);
      const result = { annee: year };
      
      yearResults.forEach(r => {
        if (r.type === 'Sans neige - Sans Voie lactée') result.sans_neige = r.p50;
        if (r.type === 'Avec neige - Sans Voie lactée') result.avec_neige = r.p50;
        if (r.type === 'Dégagé') result.degage = r.p50;
        if (r.type === 'Nuageux') result.nuageux = r.p50;
      });
      
      return result;
    });
    
    setP50Evolution(p50EvolutionData);
    addLog(`Évolution P50 calculée pour ${p50EvolutionData.length} années`);
  };
                    ...
                    {cloudEffect && cloudEffect.length > 0 && (
                      <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem' }}>
                        ...
                        <div style={{ flex: '0 0 calc(50% - 0.5rem)', padding: '1.5rem', background: 'rgba(20, 25, 45, 0.6)', border: '1px solid rgba(100, 200, 255, 0.3)', borderRadius: '4px' }}>
                          ...
                          <div style={{ marginBottom: '1rem', display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                            {cloudEffect.map(d => (
                              <button
                                key={d.annee}
                                onClick={() => {
                                  if (hiddenYears.includes(d.annee)) {
                                    setHiddenYears(hiddenYears.filter(y => y !== d.annee));
                                  } else {
                                    setHiddenYears([...hiddenYears, d.annee]);
                                  }
                                }}
                                style={{
                                  padding: '0.25rem 0.5rem',
                                  background: hiddenYears.includes(d.annee) ? 'rgba(100, 100, 100, 0.3)' : 'rgba(100, 200, 255, 0.2)',
                                  border: `1px solid ${hiddenYears.includes(d.annee) ? 'rgba(100, 100, 100, 0.5)' : 'rgba(100, 200, 255, 0.5)'}`,
                                  borderRadius: '3px',
                                  color: hiddenYears.includes(d.annee) ? 'rgba(224, 230, 237, 0.5)' : 'rgba(224, 230, 237, 0.9)',
                                  fontSize: '0.7rem',
                                  cursor: 'pointer',
                                  textDecoration: hiddenYears.includes(d.annee) ? 'line-through' : 'none'
                                }}
                              >
                                {d.annee}
                              </button>
                          ...
                          <ResponsiveContainer width="100%" height={280}>
                            ...
                            >
                              ...
                              <XAxis 
                                dataKey="annee"
                                stroke="rgba(224, 230, 237, 0.6)"
                                tick={{ fill: 'rgba(224, 230, 237, 0.8)', fontSize: 11 }}
                                angle={-45}
                                textAnchor="end"
                                height={60}
                              />
                              <YAxis 
                                stroke="rgba(224, 230, 237, 0.6)"
                                tick={{ fill: 'rgba(224, 230, 237, 0.8)', fontSize: 11 }}
                                label={{ value: 'Différence', angle: -90, position: 'insideLeft', fill: 'rgba(224, 230, 237, 0.7)', fontSize: 11 }}
                              />
                              <Tooltip 
                                contentStyle={{ 
                                ...
                                labelStyle={{ color: 'rgba(100, 200, 255, 0.9)', fontWeight: 600 }}
                                itemStyle={{ color: 'rgba(224, 230, 237, 0.95)' }}
                                formatter={(value) => {
                                  const sign = value >= 0 ? '+' : '';
                                  return `${sign}${value.toFixed(3)} mag/arcsec²`;
                                }}
                              />
                              <Bar dataKey="effet" radius={[4, 4, 0, 0]}>
                                {cloudEffect.filter(d => !hiddenYears.includes(d.annee)).map((entry, index) => (
                                  <Cell 
                                    ...
                                    stroke={entry.effet >= 0 ? 'rgba(100, 200, 255, 0.5)' : 'rgba(100, 200, 255, 0.6)'}
                                    strokeWidth={2}
                                  />
                                ))}
                              </Bar>
                          ...
                          <div style={{ marginTop: '1rem', fontSize: '0.7rem', color: 'rgba(224, 230, 237, 0.6)', display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                              <div style={{ width: '16px', height: '10px', background: 'rgba(0, 0, 0, 0.85)', border: '2px solid rgba(100, 200, 255, 0.5)', borderRadius: '2px' }}></div>
                              <span>Assombrit (+)</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                              <div style={{ width: '16px', height: '10px', background: 'rgba(255, 255, 255, 0.9)', border: '2px solid rgba(100, 200, 255, 0.6)', borderRadius: '2px' }}></div>
                              <span>Éclaircit (-)</span>
                            </div>
                          </div>
                        </div>
                        ...
                        {p50Evolution && (
                          <div style={{ flex: '0 0 calc(50% - 0.5rem)', padding: '1.5rem', background: 'rgba(20, 25, 45, 0.6)', border: '1px solid rgba(100, 200, 255, 0.3)', borderRadius: '4px' }}>
                            ...
                            <div style={{ marginBottom: '1rem', display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                              {p50Evolution.map(d => (
                                <button
                                  key={d.annee}
                                  onClick={() => {
                                    if (hiddenYearsP50.includes(d.annee)) {
                                      setHiddenYearsP50(hiddenYearsP50.filter(y => y !== d.annee));
                                    } else {
                                      setHiddenYearsP50([...hiddenYearsP50, d.annee]);
                                    }
                                  }}
                                  style={{
                                    padding: '0.25rem 0.5rem',
                                    background: hiddenYearsP50.includes(d.annee) ? 'rgba(100, 100, 100, 0.3)' : 'rgba(102, 187, 106, 0.2)',
                                    border: `1px solid ${hiddenYearsP50.includes(d.annee) ? 'rgba(100, 100, 100, 0.5)' : 'rgba(102, 187, 106, 0.5)'}`,
                                    borderRadius: '3px',
                                    color: hiddenYearsP50.includes(d.annee) ? 'rgba(224, 230, 237, 0.5)' : 'rgba(224, 230, 237, 0.9)',
                                    fontSize: '0.7rem',
                                    cursor: 'pointer',
                                    textDecoration: hiddenYearsP50.includes(d.annee) ? 'line-through' : 'none'
                                  }}
                                >
                                  {d.annee}
                                </button>
                            ...
                            <ResponsiveContainer width="100%" height={280}>
                              <BarChart
                                data={p50Evolution.filter(d => !hiddenYearsP50.includes(d.annee))}
                                margin={{ top: 20, right: 10, left: 10, bottom: 60 }}
                              >
                                <CartesianGrid strokeDasharray="3 3" stroke="rgba(100, 200, 255, 0.1)" />
                                <XAxis 
                                ...
                                <YAxis 
                                  stroke="rgba(224, 230, 237, 0.6)"
                                  tick={{ fill: 'rgba(224, 230, 237, 0.8)', fontSize: 11 }}
                                  label={{ value: 'P50 (mag/arcsec²)', angle: -90, position: 'insideLeft', fill: 'rgba(224, 230, 237, 0.7)', fontSize: 11 }}
                                  domain={[p50YLimits.min, p50YLimits.max]}
                                />
                                <Tooltip 
                                  contentStyle={{ 
                                    background: 'rgba(20, 25, 45, 0.95)', 
                                  ...
                                  labelStyle={{ color: 'rgba(102, 187, 106, 0.9)', fontWeight: 600 }}
                                  itemStyle={{ color: 'rgba(224, 230, 237, 0.95)', fontSize: '0.85rem' }}
                                  formatter={(value) => value ? value.toFixed(3) : 'N/A'}
                                />
                                {selectedCondition === 'sans_neige' && <Bar dataKey="sans_neige" fill="rgba(102, 187, 106, 0.8)" name="Sans neige" />}
                                {selectedCondition === 'avec_neige' && <Bar dataKey="avec_neige" fill="rgba(255, 255, 255, 0.9)" name="Avec neige" />}
                                {selectedCondition === 'degage' && <Bar dataKey="degage" fill="rgba(100, 200, 255, 0.8)" name="Dégagé" />}
                                {selectedCondition === 'nuageux' && <Bar dataKey="nuageux" fill="rgba(158, 158, 158, 0.8)" name="Nuageux" />}
                              </BarChart>
                            </ResponsiveContainer>
                            
                            {/* Limites axe Y */}
                            <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                <label style={{ fontSize: '0.7rem', color: 'rgba(224, 230, 237, 0.7)' }}>Min:</label>
                                <input type="number" step="0.5" value={p50YLimits.min} onChange={(e) => setP50YLimits({ ...p50YLimits, min: parseFloat(e.target.value) })} style={{ width: '50px', padding: '0.2rem', background: 'rgba(30,35,60,0.8)', border: '1px solid rgba(100, 200, 255, 0.3)', borderRadius: '3px', color: 'rgba(224, 230, 237, 0.9)', fontSize: '0.7rem' }} />
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                <label style={{ fontSize: '0.7rem', color: 'rgba(224, 230, 237, 0.7)' }}>Max:</label>
                                <input type="number" step="0.5" value={p50YLimits.max} onChange={(e) => setP50YLimits({ ...p50YLimits, max: parseFloat(e.target.value) })} style={{ width: '50px', padding: '0.2rem', background: 'rgba(30,35,60,0.8)', border: '1px solid rgba(100, 200, 255, 0.3)', borderRadius: '3px', color: 'rgba(224, 230, 237, 0.9)', fontSize: '0.7rem' }} />
                              </div>
                            </div>
